...
function initValidacaoFormularioDivulgacao() {
  ...
  const validadores = {
    ...
    "tel-usuario": function (input) {
      const valor = input.value.trim();
      if (valor.length === 0) return "Digite seu telefone.";
      const digitos = valor.replace(/\D/g, "");
      if (digitos.length !== 11) {
        return "Telefone inválido. Digite o DDD + 9 dígitos (11 números no total).";
      }
      return null;
    },
  ...
  function validarCampo(input) {
    ...
    // Campo opcional (se algum dia houver um) e ainda vazio: não é nem erro
    // nem sucesso, então não mostramos nenhuma borda/mensagem de "Tudo certo!".
